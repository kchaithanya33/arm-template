...
def deploy_application(deployment_request):
    # ------------------------------------------
    # Validate/Create Main Resource Group
    # ------------------------------------------
    ...
    # ------------------------------------------
    function_result = process_function_app(
        deployment_request
    )
    ...
    if deployment_request.storage.mode == "new":
        ...
        storage_deployment_result = create_resource_group_deployment(
            resource_group_name=deployment_request.storage.resourceGroup.name,
    ...
    logic_deployment_result = create_resource_group_deployment(
        resource_group_name=deployment_request.logicApp.resourceGroup.name,
    ...
    # Deploy Function App
    # ==================================================
    function_parameter_file = generate_parameters_file(
        deployment_request
    )
    function_deployment_result = create_resource_group_deployment(
        resource_group_name=deployment_request.functionApp.resourceGroup.name,
        parameter_file=function_parameter_file
    )
...
# FUNCTION APP
# ======================================================
def process_function_app(deployment_request):
    subscription_id = deployment_request.subscriptionId
    function = deployment_request.functionApp
    function_rg = function.resourceGroup
    ...
    if function_rg.mode == "new":
        ...
        create_resource_group_template(
            location=function_rg.location,
            parameter_file=rg_parameter_file
        )
    elif function_rg.mode == "existing":
        access = validate_resource_group_access(
            subscription_id,
            function_rg.name
        )
    ...
    return {
        "name": function.name,
        "mode": "new",
        "resourceGroup": function_rg.name,
        "storageAccount": storage.name,
        "runtimeStack": function.runtimeStack,
